...
-- Đổi tên hàm thành LexusFastTick để tương thích với vòng lặp ReceiveTick mượt mà
_G.LexusFastTick = function(DeltaSeconds)
    ...
    -- ==========================================
    -- [LUỒNG CHẬM] - 2 Giây/lần: Tìm địch, Mod súng, Mod xe
    -- ==========================================
    if currentTime - _G.LexusLastScan > 2.0 then
        _G.LexusLastScan = currentTime

        -- 1. Mod Súng (Giữ nguyên các thông số mới của bạn)
        ...
        -- 2. Mod Xe (Giữ nguyên tối ưu bơm xăng của bạn)
            ...
            if slua.isValid(CurrentVehicle) then
                ...
                if slua.isValid(VehicleCommon) then
                    if VehicleCommon.Fuel < 10.0 or (type(VehicleCommon.NoFuel) == "function" and VehicleCommon:NoFuel()) then
                        ...
                        if type(VehicleCommon.OnRep_Fuel) == "function" then
        ...
        -- 3. Quét Radar tìm Địch
        -- Xoá toàn bộ chấm cũ cho sạch bản đồ
        ...
        if CharacterClass then
            ...
            for i = 0, outActors:Num() - 1 do
                ...
                -- ĐÃ FIX: Chặn tự đánh dấu bản thân và đồng đội
                if slua.isValid(enemy) and enemy.PlayerKey ~= myKey and enemy.TeamID ~= uPlayerCharacter.TeamID then
                    ...
                    -- ĐÃ FIX: Dùng biến 'i' thay cho số 4 để không bị giới hạn 4 người
    ...
    -- ==========================================
    -- [LUỒNG NHANH] - Mỗi khung hình: Vẽ Box ESP và di chuyển Radar mượt mà
    -- ==========================================
    ...
    for _, enemy in pairs(_G.LexusEnemyCache) do
        if slua.isValid(enemy) then
            ...
            if isAlive then
                ...
                -- Cập nhật toạ độ Mark liên tục (Smooth Minimap Radar)
                ...
                -- ĐÃ FIX: Duration = 0.0 để ESP di chuyển mượt mà không bị delay/overlap
                if slua.isValid(USTExtraGameplayStatics) then
                    ...
                    -- ĐÃ FIX LỖI TEXT: Thêm biến 'enemy' vào để chữ bám dính vào mục tiêu
